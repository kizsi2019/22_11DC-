...
print(f'A neme: {"férfi" if get_gender(first_name)[0] == "male" else "nő"}, 'f'ennek valószínűsége: {get_gender(first_name)[1] * 100:.0f}%.')
print(f'A kor valószínűleg {get_age(first_name)} év.')
print(f'Az ország kódja: {get_nat(first_name)[0]}, 'f'ennek valószínűsége: {get_nat(first_name)[1]* 100:.0f}%.')
